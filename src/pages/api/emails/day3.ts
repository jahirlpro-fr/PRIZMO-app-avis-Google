...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        // Cherche les marchands inscrits il y a exactement 3 jours
        const now = new Date();
              ...
              <table width="100%" style="max-width:580px;background:white;border-radius:24px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.06);">
                ...
                <tr>
                  <td style="padding:40px;">
                    ...
                    ${[
                        { num: "01", emoji: "🗣️", title: "La phrase magique au comptoir", desc: "\"Avant de partir, vous pouvez tourner notre roue de la fortune et tenter de gagner un cadeau — il suffit de laisser un avis Google d'abord !\" Simple, naturel, efficace." },
                        { num: "02", emoji: "⏰", title: "Le bon moment : l'addition", desc: "Le client est satisfait, il paye. C'est LE moment idéal. Pas en début de repas, pas quand il est pressé — à l'addition, quand l'expérience est fraîche." },
                        { num: "03", emoji: "📍", title: "Placez l'affiche au niveau des yeux", desc: "Sur le comptoir de caisse, collée sur le terminal de paiement, ou sur la table. Le QR code doit être visible sans chercher." },
                    ].map(tip => `
                      <div style="display:flex;gap:16px;margin-bottom:24px;padding-bottom:24px;border-bottom:1px solid #f0f0f0;">
                        <div style="flex-shrink:0;width:36px;height:36px;background:linear-gradient(135deg,#f59e0b,#ef4444);border-radius:10px;color:white;font-size:12px;font-weight:800;text-align:center;line-height:36px;">${tip.num}</div>
                        <div>
                    ...
                    <div style="background:#f0fdf4;border-left:4px solid #22c55e;border-radius:0 12px 12px 0;padding:16px 20px;margin-bottom:28px;">
                      ...
                      <p style="color:#166534;font-size:13px;line-height:1.6;margin:0;">
                        Les commerçants qui briefent leur équipe génèrent en moyenne <strong>3x plus d'avis</strong> que ceux qui misent uniquement sur l'affiche.
                      </p>
                    </div>
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0">
                      <tr>
                        <td align="center">
                          <a href="https://prizmo.pro/admin" style="background:linear-gradient(135deg,#f59e0b,#ef4444);color:white;padding:16px 36px;border-radius:100px;font-size:15px;font-weight:700;text-decoration:none;display:inline-block;">
                    ...
                    <p style="color:#aaa;font-size:13px;text-align:center;margin:24px 0 0;">Une question ? Répondez directement à cet email.</p>
                  </td>
                </tr>
        ...
        console.error("Day3 cron error:", error);
        return res.status(500).json({ error: "Erreur cron day3" });
    }
}